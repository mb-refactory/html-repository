...
function updateGrid(data) {
    ...
    podcastInfo.feeds.forEach(feed => {
        ...
        link.addEventListener('click', function () {
            sessionStorage.setItem('selectedPodcast', JSON.stringify(feed));
        });
...
// Seleziona le top 3 categorie tra tutti i podcast a cui si è iscritti 
const categories = [];
let topCategories = [];
fetchSubscribedPodcasts(subscribedIDs).then((subscribedPodcasts) => {
    ...
    podcastIndexTrendingAPI(12, topCategories)
        // Infine viene aggiornata la griglia contenente 
        // tutti i podcast consigliati in base alle categorie 
        // dei podcast a cui si è iscritti
